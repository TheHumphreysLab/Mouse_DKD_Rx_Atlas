...
checkRsubreadVersion<- function(){
  if(length(grep("Rsubread",installed.packages()))==0){
    ...
    BiocManager::install("Rsubread",dependencies = TRUE, ask = FALSE)
  }else{
...
runFeatureCount<-function(abamfile,RG,saf,strand,type,primaryOnly,cpu,mem){
  ...
  fc<-Rsubread::featureCounts(files=abamfile,
                                   annot.ext=saf,
  ...
  file_names<-unlist(strsplit(abamfile, ".", fixed = TRUE))
  write.table(fc$stat, paste0(file_names[1],".",type,".stat.txt"))
  write.table(x=data.frame(fc$annotation[,c("GeneID","Length")],fc$counts,stringsAsFactors=FALSE),file=paste0(file_names[1],".",type,".counts.txt"),quote=FALSE,sep="\t",row.names=FALSE)
}
